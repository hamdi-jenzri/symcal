...
class JavaCompiledFunctionSpec extends FlatSpec with Matchers {
  ...
  it should "measure time for compiling and running a function" in {
    ...
    println(s"Best times for Java-compiled function: compilation ${compileTimes.map(_._1).min / 1000000} ms; run time ${compileTimes.map(_._2).min} ns")
    println(s"Worst times for Java-compiled function: compilation ${compileTimes.map(_._1).max / 1000000} ms; run time ${compileTimes.map(_._2).max} ns")
  }
